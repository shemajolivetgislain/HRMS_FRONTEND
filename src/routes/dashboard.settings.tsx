        ...
        >
          ...
          <div className="flex-1 min-w-0 h-full overflow-y-auto no-scrollbar bg-background/40">
            <div className="max-w-4xl p-10 space-y-12">
              ...
              <TabsContent value="tax" className="mt-0 space-y-10 animate-in fade-in slide-in-from-right-1 duration-300">
                      ...
                      <tbody className="divide-y divide-border/5">
                        {taxConfig.rraBrackets.map((b, i) => (
                          <tr key={i} className="hover:bg-muted/5 transition-colors">
                            <td className="px-6 py-4 font-semibold text-foreground/70">Level {i + 1}</td>
                            <td className="px-6 py-4 tabular-nums text-muted-foreground/60">
                              {b.min.toLocaleString()} — {b.max ? b.max.toLocaleString() : "Above"}
                            </td>
                            <td className="px-6 py-4 font-bold text-primary">{b.rate}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <section className="pt-10 border-t border-border/5 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <SettingsField label="RSSB Pension (Employer)" defaultValue={`${taxConfig.rssbEmployer}%`} />
                    <SettingsField label="Maternity Fund (Combined)" defaultValue="0.6%" />
                    <SettingsField label="CBHI Contribution" defaultValue={`${taxConfig.cbhiRate}%`} />
                  </div>
                </section>
              </TabsContent>
              ...
              <TabsContent value="policy" className="mt-0 space-y-8 animate-in fade-in slide-in-from-right-1 duration-300">
                ...
                <div className="divide-y divide-border/5 border-y border-border/5">
                  ...
                  <ToggleRow icon={File02Icon} title="Offboarding Policy" description="Require Resignation Letter and Clearance for termination." defaultChecked />
                  <ToggleRow icon={Shield01Icon} title="Identity Verification" description="Force 2FA for all users with access to financial records." defaultChecked />
                </div>
              </TabsContent>
...
function ThemeCard({ active, onClick, icon: Icon, title, description }: { active: boolean; onClick: () => void; icon: any; title: string; description: string }) {
  return (
    <button
      onClick={onClick}
      className={cn("flex flex-col gap-4 p-5 rounded-2xl border text-left transition-all duration-300", active ? "bg-primary/[0.03] border-primary/20 shadow-xs" : "bg-muted/5 border-border/5 hover:border-border/20")}
    >
      <div className={cn("size-9 rounded-xl flex items-center justify-center border transition-colors", active ? "bg-primary text-white border-primary" : "bg-muted/10 text-muted-foreground/40 border-border/5")}>
