...
export function FilterPopover() {
  return (
    <Popover>
      ...
      />
      <PopoverContent align="end" className="w-[320px] p-0 overflow-hidden rounded-xl border-border/40 shadow-xl">
        <PopoverHeader className="px-5 py-4 border-b border-border/5 bg-muted/5">
          <PopoverTitle className="text-sm font-semibold tracking-tight">Dashboard Filters</PopoverTitle>
          ...
          <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {["Engineering", "Design", "Marketing", "Sales"].map((dept) => (
                <div key={dept} className="flex items-center gap-2 group cursor-pointer">
                  <Checkbox id={dept} className="size-3.5 rounded-sm border-border/60 data-[state=checked]:bg-primary" />
                  <label htmlFor={dept} className="text-[12px] font-medium text-foreground/70 cursor-pointer group-hover:text-foreground transition-colors">
                    {dept}
                  </label>
                </div>
              ))}
        ...
        <div className="px-5 py-3.5 border-t border-border/5 bg-muted/5 flex items-center justify-between">
          <Button variant="ghost" size="sm" className="h-8 text-[11px] font-bold text-muted-foreground/60 hover:text-foreground">
            Reset
          </Button>
