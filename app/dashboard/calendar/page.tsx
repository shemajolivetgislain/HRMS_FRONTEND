            ...
            <FramePanel className="p-0 overflow-hidden h-full flex flex-col bg-card">
              <FrameHeader>
                <div>
                  ...
                  <FrameDescription>Active View: {date?.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</FrameDescription>
                </div>
                <Tabs defaultValue="day" className="h-8">
                  <TabsList className="bg-muted/20 border-border/5 p-0.5 rounded-lg h-full">
                    <TabsTrigger value="day" className="rounded-md px-3 text-[10px] font-bold capitalize tracking-tight data-[state=active]:bg-background data-[state=active]:shadow-xs">Day</TabsTrigger>
                    <TabsTrigger value="week" className="rounded-md px-3 text-[10px] font-bold capitalize tracking-tight data-[state=active]:bg-background data-[state=active]:shadow-xs">Week</TabsTrigger>
                    <TabsTrigger value="month" className="rounded-md px-3 text-[10px] font-bold capitalize tracking-tight data-[state=active]:bg-background data-[state=active]:shadow-xs">Month</TabsTrigger>
                  </TabsList>
                </Tabs>
              </FrameHeader>
              ...
              <FrameContent className="p-0 flex-1 flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-border/5 overflow-hidden">
                ...
                <div className="w-full md:w-[280px] p-6 bg-muted/[0.02] flex-shrink-0 flex flex-col gap-6">
                  ...
                  <div className="space-y-4 pt-6 border-t border-border/5">
                    <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground/30">Suggested Slots</p>
                    <div className="space-y-2">
                      {["10:00 AM", "2:30 PM", "4:15 PM"].map((slot) => (
                        <button key={slot} className="flex items-center justify-between w-full px-3 py-2 rounded-lg bg-muted/5 border border-border/5 hover:border-primary/20 hover:bg-primary/[0.02] transition-all group/slot">
                          <span className="text-[12px] font-semibold text-foreground/70">{slot}</span>
                          <HugeiconsIcon icon={Add01Icon} size={12} className="text-muted-foreground/30 group-hover/slot:text-primary transition-colors" />
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex-1 p-0 overflow-y-auto no-scrollbar bg-card/30">
                  <div className="sticky top-0 z-20 px-6 py-3 bg-card/80 backdrop-blur-md border-b border-border/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant="success" className="h-5 bg-primary/5 text-primary border-primary/10 rounded-md px-1.5 font-bold text-[9px]">TODAY</Badge>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon-sm" className="rounded-lg h-8 w-8"><HugeiconsIcon icon={Calendar03Icon} size={14} /></Button>
                      <Button variant="ghost" size="icon-sm" className="rounded-lg h-8 w-8"><HugeiconsIcon icon={MoreHorizontalIcon} size={14} /></Button>
                    </div>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className="w-[3px] h-8 rounded-full shrink-0" style={{ backgroundColor: style.color }} />
                            <div className="min-w-0">
                              <p className="text-[13px] font-semibold text-foreground/90 leading-tight truncate">{ev.title}</p>
                              <p className="text-[10px] font-medium text-muted-foreground/50 mt-1">{ev.start} — {ev.end}</p>
                            </div>
                          </div>
                          <div className="flex items-center justify-between mt-auto opacity-40 group-hover/devent:opacity-100 transition-opacity">
                             ...
                             <HugeiconsIcon icon={Tick01Icon} size={12} className="text-muted-foreground/20" />
                          </div>
                        </div>
                      );
                    })}
            ...
            <FramePanel className="p-0 overflow-hidden bg-card">
              <FrameHeader>
                <div>
                  ...
                  <FrameDescription>High importance activities</FrameDescription>
                </div>
              </FrameHeader>
              <FrameContent className="p-0">
                <div className="divide-y divide-border/5">
                  {upcomingEvents.map((event) => {
                    ...
                    return (
                      <div key={event.id} className="p-5 hover:bg-muted/5 transition-colors group/event">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex flex-col gap-2 mt-3">
                              <div className="flex items-center gap-4 text-[11px] font-bold text-muted-foreground/40 capitalize tracking-tight">
                                <span className="flex items-center gap-1.5">
                                  <HugeiconsIcon icon={Clock01Icon} size={12} strokeWidth={2.5} />
                                  {event.time}
                                </span>
                                {event.location && (
                                  <span className="flex items-center gap-1.5 truncate">
                                    <HugeiconsIcon icon={Location01Icon} size={12} strokeWidth={2.5} />
                                    {event.location}
                                  </span>
                                )}
                              </div>
                              ...
                              {event.attendees.length > 0 && (
                                <div className="flex items-center gap-1.5 pt-1">
                                  {event.attendees.map((name, i) => (
                                    <UserAvatar key={i} name={name} size="sm" className="size-5 border-2 border-card" />
                                  ))}
                          ...
                          <Button variant="ghost" size="icon-sm" className="rounded-lg opacity-0 group-hover/event:opacity-100 transition-opacity">
              ...
              <FrameFooter>
                <Button variant="outline" className="w-full h-8 text-[11px] font-bold rounded-lg border-border/40">
                  Full Schedule
                </Button>
              </FrameFooter>
            </FramePanel>
          ...
          <Frame>
            <FramePanel className="p-5 flex items-center justify-between bg-card">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[13px] font-semibold text-foreground/80 leading-none mb-1">Availability</p>
                  <p className="text-[11px] font-bold text-muted-foreground/40 capitalize tracking-widest">84% Team Active</p>
                </div>
              </div>
              <Badge variant="success" showDot className="h-5 rounded-md">Normal</Badge>
            </FramePanel>
          </Frame>
        </section>
      </div>
