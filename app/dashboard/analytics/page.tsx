            ...
            <FramePanel className="p-0 overflow-hidden h-full bg-primary/[0.02] border-primary/10">
              <FrameHeader className="border-b-0">
                <div className="flex items-center gap-2">
                  ...
                  <FrameTitle className="text-primary/80">AI Analysis</FrameTitle>
                </div>
              </FrameHeader>
                ...
                <div className="space-y-4">
                  <div className="p-3.5 rounded-xl bg-background border border-primary/5 shadow-sm">
                    <p className="text-[13px] font-medium text-foreground/80 leading-relaxed italic">
                      "Headcount is trending 4% above target. Retention in Engineering has reached an all-time high of 98% this quarter."
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="success" className="h-5 rounded-md text-[9px] font-bold">LATEST UPDATE</Badge>
                    <span className="text-[10px] font-bold text-muted-foreground/40 capitalize tracking-widest">2 hours ago</span>
                  </div>
                </div>
              </FrameContent>
            </FramePanel>
            ...
            <FramePanel className="p-0 overflow-hidden bg-card">
              <FrameHeader>
                <div>
                  ...
                  <FrameDescription>Comparative analysis of actual growth vs quarterly targets</FrameDescription>
                </div>
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-2">
                    <div className="size-1.5 rounded-full bg-primary shadow-[0_0_8px_var(--primary)]" />
                    <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest">Actual Growth</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="size-1.5 rounded-full bg-chart-2 opacity-50" />
                    <span className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest">Target Baseline</span>
                  </div>
                </div>
              </FrameHeader>
                ...
                <ChartContainer config={chartConfig} className="h-full w-full">
                  <AreaChart data={growthData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <XAxis 
                      ...
                      axisLine={false} 
                      tickLine={false} 
                      tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))", fontWeight: 600, opacity: 0.4 }}
                      tickMargin={12}
                    />
                    <YAxis 
                      axisLine={false} 
                    ...
                    <ChartTooltip 
                      ...
                      content={<ChartTooltipContent className="rounded-xl border-border/10 shadow-xl" />} 
                    />
                    <Area
                      type="monotone"
                      dataKey="headcount"
            ...
            <FramePanel className="p-0 overflow-hidden bg-card">
              <FrameHeader>
                <div>
                  ...
                  <FrameDescription>Benchmarked engagement scores by team</FrameDescription>
                </div>
              </FrameHeader>
              <FrameContent className="h-[320px] py-6">
                <ChartContainer config={chartConfig} className="h-full w-full">
                  <BarChart data={deptData} layout="vertical" margin={{ top: 0, right: 40, left: 0, bottom: 0 }} barCategoryGap="25%">
                    ...
                    <YAxis 
                      ...
                      axisLine={false} 
                      tickLine={false}
                      tick={{ fontSize: 11, fontWeight: 600, fill: "hsl(var(--foreground))", opacity: 0.6 }}
                      width={100}
                    />
                    <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                    <Bar dataKey="happiness" radius={[0, 6, 6, 0]}>
                      {deptData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.happiness >= 90 ? "var(--primary)" : "var(--chart-1)"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ChartContainer>
            ...
            <FramePanel className="p-0 overflow-hidden bg-card">
              <FrameHeader>
                <div>
                  ...
                  <FrameDescription>Productivity ratings relative to team capacity</FrameDescription>
                </div>
              </FrameHeader>
              <FrameContent className="h-[320px] py-6">
                <ChartContainer config={chartConfig} className="h-full w-full">
                  <BarChart data={deptData} layout="vertical" margin={{ top: 0, right: 40, left: 0, bottom: 0 }} barCategoryGap="25%">
                    ...
                    />
                    <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                    <Bar dataKey="productivity" fill="var(--chart-3)" radius={[0, 6, 6, 0]} />
                  </BarChart>
                </ChartContainer>
              </FrameContent>
            </FramePanel>
...
function AnalyticsStatCard({ label, value, change, up, sub, icon: Icon }: { label: string; value: string; change: string; up: boolean; sub: string; icon: any }) {
  return (
    <Frame className="h-full group/stat">
      <FramePanel className="p-5 flex flex-col justify-between h-full bg-card">
        <div className="flex items-center justify-between">
          ...
          >
            <span className="text-[10px]">{up ? "↑" : "↓"} {change}</span>
          </Badge>
        </div>
        <div className="mt-6">
          ...
          <p className="text-[11px] font-bold text-muted-foreground/40 capitalize tracking-widest mt-2">{label}</p>
          <p className="text-[10px] font-medium text-muted-foreground/30 mt-1 flex items-center gap-1">
            <HugeiconsIcon icon={InformationCircleIcon} size={10} strokeWidth={2.5} />
            {sub}
          </p>
        </div>
      </FramePanel>
