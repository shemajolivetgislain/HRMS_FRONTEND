            ...
            <FramePanel className="p-0 overflow-hidden">
              <FrameHeader className="border-b-0 pb-2">
                <div>
                  ...
                  <FrameDescription>Manage and view all employee profiles</FrameDescription>
                </div>
              ...
              <div className="px-6 pb-5 pt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 border-b border-border/5">
                ...
                >
                  <SelectTrigger className="h-9 rounded-lg border-border/40 bg-muted/5">
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground/40 text-[10px] font-bold uppercase tracking-wider">Dept:</span>
                      <SelectValue placeholder="All Departments" className="text-xs" />
                    </div>
                  </SelectTrigger>
                  <SelectContent>
                ...
                >
                  <SelectTrigger className="h-9 rounded-lg border-border/40 bg-muted/5">
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground/40 text-[10px] font-bold uppercase tracking-wider">Status:</span>
                      <SelectValue placeholder="All Status" className="text-xs" />
                    </div>
                  </SelectTrigger>
                  <SelectContent>
                    {statuses.map((status) => (
              ...
              {selectedIds.size > 0 && (
                <div className="px-6 py-3 bg-primary/[0.02] border-b border-primary/10 flex items-center justify-between animate-in fade-in slide-in-from-top-1 duration-200">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="h-8 px-3 rounded-lg border-primary/20 text-primary text-[11px] font-bold hover:bg-primary/5">
                      <HugeiconsIcon icon={Mail01Icon} className="size-3.5 mr-1.5" />
                      Email
                    </Button>
                    <Button
                      variant="outline"
                ...
                <Table>
                  <TableHeader className="bg-muted/10">
                    <TableRow className="hover:bg-transparent border-border/5">
                      ...
                      <TableHead className="text-[11px] font-bold text-muted-foreground/40 capitalize tracking-widest px-2">Employee</TableHead>
                      <TableHead className="text-[11px] font-bold text-muted-foreground/40 capitalize tracking-widest px-2">Department</TableHead>
                      <TableHead className="text-[11px] font-bold text-muted-foreground/40 capitalize tracking-widest px-2">Position</TableHead>
                  ...
                  <TableBody>
                    {filteredUsers.length > 0 ? (
                      filteredUsers.map((user) => (
                        <TableRow key={user.id} className="border-border/5 hover:bg-muted/5 transition-colors group">
                          ...
                          <TableCell className="px-2">
                            <span className="text-[13px] font-medium text-foreground/70">{user.department}</span>
                          </TableCell>
                          <TableCell className="px-2">
                            <span className="text-[13px] font-medium text-muted-foreground/70">{user.position}</span>
                          </TableCell>
                          <TableCell className="px-2">
                            <Badge 
                              variant={
                          ...
                          <TableCell className="text-right pr-6">
                            <DropdownMenu>
                              <DropdownMenuTrigger
                                render={
                                  <Button variant="ghost" size="icon-sm" className="rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                                    <HugeiconsIcon icon={MoreHorizontalIcon} className="size-4 opacity-50" />
                                  </Button>
                                }
                              />
                              <DropdownMenuContent align="end" className="w-48 rounded-xl border-border/40 shadow-xl">
                                <DropdownMenuItem render={<Link href={`/dashboard/employees/${user.id}`} />}>
                                ...
                                <DropdownMenuItem>
                                  <HugeiconsIcon icon={LockPasswordIcon} className="size-4 mr-2" />
                        ...
                        <TableCell colSpan={6} className="h-[300px] text-center">
                          <div className="flex flex-col items-center justify-center gap-3">
                            <div className="h-12 w-12 rounded-2xl bg-muted/10 flex items-center justify-center">
                              <HugeiconsIcon icon={UserGroupIcon} className="size-6 text-muted-foreground/20" strokeWidth={1.5} />
                            </div>
                            <div className="space-y-1">
                              <p className="text-[13px] font-semibold text-foreground/70">No employees found</p>
                              <p className="text-[11px] text-muted-foreground/50 max-w-[200px] mx-auto">
                                We couldn't find any records matching your current search or filters.
                              </p>
                            </div>
                            <Button 
                              variant="outline" 
              ...
              <FrameFooter className="flex items-center justify-between border-t border-border/5">
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-2 border-l border-border/5 pl-6">
                    <span className="text-[10px] text-muted-foreground/40 font-bold capitalize tracking-widest">Rows:</span>
